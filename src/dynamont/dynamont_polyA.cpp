...
void logF(const double* sig, const double* S, const double* L, const double* A, const double* PA, const double* TR, const size_t T){
    ...
    for (size_t t=1; t<T; ++t){
        ...
        start = logPlus(start, S[t-1] + log_normal_pdf(sig[t-1], TODO_mean, TODO_stdev) + s);

        leader = logPlus(leader, S[t-1] + log_normal_pdf(sig[t-1], TODO_mean, TODO_stdev) + l1);
        leader = logPlus(leader, L[t-1] + log_normal_pdf(sig[t-1], TODO_mean, TODO_stdev) + l2);

        adapter = logPlus(adapter, L[t-1] + log_normal_pdf(sig[t-1], TODO_mean, TODO_stdev) + a1);
        adapter = logPlus(adapter, A[t-1] + log_normal_pdf(sig[t-1], TODO_mean, TODO_stdev) + a2);

        polya = logPlus(polya, A[t-1] + log_normal_pdf(sig[t-1], TODO_mean, TODO_stdev) + pa1);
        polya = logPlus(polya, PA[t-1] + log_normal_pdf(sig[t-1], TODO_mean, TODO_stdev) + pa2);

        transcript = logPlus(transcript, PA[t-1] + log_normal_pdf(sig[t-1], TODO_mean, TODO_stdev) + tr1);
        transcript = logPlus(transcript, TR[t-1] + log_normal_pdf(sig[t-1], TODO_mean, TODO_stdev) + tr2);
...
void logB(const double* sig, const double* S, const double* L, const double* A, const double* PA, const double* TR, const size_t T) {
    ...
    for (size_t t=T; t-->0;){ // T-1, ..., 1, 0
        ...
        start = logPlus(start, S[t+1] + log_normal_pdf(sig[t], TODO_mean, TODO_stdev) + s);
        start = logPlus(start, L[t+1] + log_normal_pdf(sig[t], TODO_mean, TODO_stdev) + l1);

        leader = logPlus(leader, S[t+1] + log_normal_pdf(sig[t], TODO_mean, TODO_stdev) + l2);
        leader = logPlus(leader, A[t+1] + log_normal_pdf(sig[t], TODO_mean, TODO_stdev) + a1);

        adapter = logPlus(adapter, A[t+1] + log_normal_pdf(sig[t], TODO_mean, TODO_stdev) + a2);
        adapter = logPlus(adapter, PA[t+1] + log_normal_pdf(sig[t], TODO_mean, TODO_stdev) + pa1);

        polya = logPlus(polya, PA[t+1] + log_normal_pdf(sig[t], TODO_mean, TODO_stdev) + pa2);
        polya = logPlus(polya, TR[t+1] + log_normal_pdf(sig[t], TODO_mean, TODO_stdev) + tr1);
        
        transcript = logPlus(transcript, TR[t+1] + log_normal_pdf(sig[t], TODO_mean, TODO_stdev) + tr2);
