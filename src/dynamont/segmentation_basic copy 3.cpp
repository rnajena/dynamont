...
void logB(double* sig, int* kmer_seq, double* M, double* E, const int &T, const int &N, vector<tuple<double, double>>* model) {
    ...
    for(int t=T-2; t>=0; t--){
        for(int n=N-2; n>=0; n--){
            ...
            if (t<T-2-C && n<N-2) {
                tmp=M[(t+1+C)*N+(n+1)] + scoreKmer(sig[t+1], kmer_seq[n+1], model) + m1;
                for (int l=1; l<=C; l++){
                    tmp+=scoreKmer(sig[t+l+1], kmer_seq[n+1], model);
                }
                ext=logPlus(ext, tmp);
            ...
            if (t<T-2) {
                mat=logPlus(mat, E[(t+1)*N+n] + scoreKmer(sig[t+1], kmer_seq[n], model)  + e1); // e1 first extend
                ext=logPlus(ext, E[(t+1)*N+n] + scoreKmer(sig[t+1], kmer_seq[n], model) + e2); // e2 extend further
                ext=logPlus(ext, E[(t+1)*N+n] + error(sig[t+1]) + e3); // e3 error
            }
    ...
    for(int t=1; t<T-1; t++){
        for(int n=1; n<N-1; n++){
            if (t-C+1>=0 && t-C+1<=T-1) {
                ...
                tempM = forE[(t-C)*N+n] + m1 + scoreKmer(sig[t-1], kmer_seq[n-1], model) + backM[t*N+n];
                for(int l=1; l<=C; l++){
                    tempM+=scoreKmer(sig[t+l-1], kmer_seq[n-1], model);
                }
                newM1 = logPlus(newM1, tempM);
            }
            newE1 = logPlus(newE1, forM[t*N+n] + e1 + scoreKmer(sig[t-1], kmer_seq[n-1], model) + backE[t*N+(n-1)]);
            newE2 = logPlus(newE2, forE[t*N+n] + e2 + scoreKmer(sig[t-1], kmer_seq[n-1], model) + backE[t*N+(n-1)]);
            newE3 = logPlus(newE3, forE[t*N+n] + e3 + error(sig[t-1])                           + backE[t*N+(n-1)]);
        }
    }
...
tuple<double*, double*> trainEmission(double* sig, int* kmer_seq, double* forM, double* forE, double* backM, double* backE, const int &T, const int &N, vector<tuple<double, double>>* model) {
    ...
    double* G = new double[(T-1)*(N-1)];
    fill_n(G, (T-1)*(N-1), -INFINITY);
    // double* GM = new double[(T-1)*(N-1)];
    // fill_n(GM, (T-1)*(N-1), -INFINITY);
    // double* GE = new double[(T-1)*(N-1)];
    // fill_n(GE, (T-1)*(N-1), -INFINITY);
    ...
    for(int t=0; t<T-1; t++){
        ...
        // double sE = -INFINITY;
        for(int n=0; n<N-1; n++){
            // G[t*(N-1)+n] = logPlus(forM[(t+1)*N+(n+1)], forE[(t+1)*N+(n+1)]) + logPlus(backM[t*N+n], backE[t*N+n]);
            // G[t*(N-1)+n] = logPlus(forM[(t+1)*N+(n+1)], forE[(t+1)*N+(n+1)]) + logPlus(backM[t*N+n], backE[t*N+n]);
            // GM[t*(N-1)+n] = forM[(t+1)*N+(n+1)] + backM[t*N+n];
            // GE[t*(N-1)+n] = forE[(t+1)*N+(n+1)] + backE[t*N+n];
            
            // TODO is this korrekt?
            G[t*(N-1)+n] = logPlus(forM[(t+1)*N+(n+1)] + backM[t*N+n], forE[(t+1)*N+(n+1)] + backE[t*N+n]);
            
            // G[t*(N-1)+n] = logPlus()
            // G[t*N+n] = forE[(t+1)*N+(n+1)] + backE[t*N+n];
            // G[t*N+n] = forM[(t+1)*N+(n+1)] + backM[t*N+n] + forE[(t+1)*N+(n+1)] + backE[t*N+n];

            s = logPlus(s, G[t*(N-1)+n]);
            // sM = logPlus(sM, GM[t*(N-1)+n]);
            // sE = logPlus(sE, GE[t*(N-1)+n]);
        }
        for(int n=0; n<N-1; n++){
            if (!isinf(s)) {
                G[t*(N-1)+n] -= s;
    ...
    for (int n=0; n<N-1; n++) {
        counts[kmer_seq[n]]++;
        for (int t=0; t<T-1; t++) {
            kmers[n] += exp(G[t*(N-1)+n]) * sig[t];
            d[n] += exp(G[t*(N-1)+n]);

            // kmers[n] += (exp(GM[t*(N-1)+n] + GE[t*(N-1)+n])) * sig[t];
            // d[n] += exp(GM[t*(N-1)+n] + GE[t*(N-1)+n]);
        }
        kmers[n] = kmers[n] / d[n];
    }
    ...
    for (int n=0; n<N-1; n++) {
        for (int t=0; t<T-1; t++) {
            kmers[n] += exp(G[t*(N-1)+n]) * abs(sig[t] - means[kmer_seq[n]]);
            // kmers[n] += (exp(GM[t*(N-1)+n] + GE[t*(N-1)+n])) * abs(sig[t] - means[kmer_seq[n]]);
        }
        kmers[n] = kmers[n] / d[n];
    }
...
void printTrainedTransitionParams(double* sig, int* kmer_seq, double* forM, double* forE, double* backM, double* backE, const int &T, const int &N, vector<tuple<double, double>>* model) {
    ...
    cout<<"m1:"<<exp(m1)<<";e1:"<<exp(e1)<<";e2:"<<exp(e2)<<";e3:"<<exp(e3)<<endl;
...
int main(int argc, char* argv[]) {
    ...
    while(truish) {
        ...
        // This checks out
        if (abs(forE[T*N-1] - backM[0])>EPSILON || isinf(backM[0]) || isinf(forE[T*N-1])) {
            cout<<"Z values between matrices do not match! forE: "<<forE[T*N-1]<<", backM: "<<backM[0]<<"\n";
            cout.flush();
            exit(11);
        }
        ...
        //     printTrainedTransitionParams(sig, kmer_seq, forM, forE, backM, backE, T, N, &model);
            ...
            double* LPM = logP(forM, backM, backM[0], T, N); // log probs
            double* LPE = logP(forE, backE, backM[0], T, N); // log probs
