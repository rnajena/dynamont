...
def train(rawdatapath : str, fastqpath : str, polya : dict, outdir : str, batch_size : int, epochs :int, param_file : str) -> None:
    ...
    for e in range(epochs):
        for file in files:
            ...
            for readid in r5:
                ...
                if not readid in polya or polya[readid] < 1:
                    continue
                i+=1
                signal = r5.getpASignal(readid)[polya[readid]:] # cut off prefix (adapter, polyA) part of the signal and only segment read part
                print('i:', i, 'read:', readid, 'signal length', len(signal), 'read length', len(basecalls[readid][::-1]))
...
def main() -> None:
    ...
    train(args.raw, args.fastq, polya, args.out, args.batch_size, args.epochs, param_file)
