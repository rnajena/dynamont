...
Using this script, we reduce the 9mer models to smaller kmer models by only using the middle k bases.
e.g. k=5 position-wise conversion of 9mer to 5mer:
012345678 -> 23456
"""
...
def parse() -> Namespace:
    ...
    parser.add_argument("outsize", default=5, type=int, help="Size of output kmer")
...
def reduce(inmodel : dict, k : int) -> dict:
    outmodel = {}
    offset = (len(next(iter(inmodel))) - k) // 2  # Center the k-mer selection

    for kmer in inmodel:

        rKmer = kmer[offset : offset + k]
    ...
    for kmer in outmodel:
        outmodel[kmer] = [
            np.mean(outmodel[kmer][0], dtype=np.float128),
            np.mean(outmodel[kmer][1], dtype=np.float128),
        ]
...
def main() -> None:
    ...
    outmodel = reduce(model, args.outsize)
